...
public:
  SignatureHelpCollector(const clang::CodeCompleteOptions &CodeCompleteOpts,
                         ...
                         SignatureSkepuHelp &SigSkepu,
                         std::vector<std::string>& SkeletonTypes,
                        std::unordered_map<std::string, Skeleton>& SkeletonDescriptions)
      : CodeCompleteConsumer(CodeCompleteOpts), SigHelp(SigHelp), SigSkepu(SigSkepu),
        SkeletonTypes(SkeletonTypes), SkeletonDescriptions(SkeletonDescriptions),
  ...
  void ProcessOverloadCandidates(Sema &S, unsigned CurrentArg,
    ...
    std::vector<FunctionDecl*> ScoredSignaturesFunctions;
    ScoredSignaturesFunctions.reserve(NumCandidates);
    ...
    for (unsigned I = 0; I < NumCandidates; ++I) {
      ...
      ScoredSignaturesFunctions.push_back(Candidate.getFunction());
    }
    ...
    for (std::size_t i{0}; i < ScoredSignatures.size(); ++i){
      auto SS = ScoredSignatures[i];
      auto fd = ScoredSignaturesFunctions[i];
      OverloadCandidate Candidate = Candidates[i];

      bool useSkepu = true;
      //clangd::log("trying to use codecomplete {0}", fd->getQualifiedNameAsString());

      if(fd && useSkepu)
      {
        if(fd->getQualifiedNameAsString().find("skepu::") != std::string::npos) 
        {

          //clangd::log("fd {0}, {1}", fd->getQualifiedNameAsString(), fd->getNameAsString());
          //Is a skeleton Instantiation
          bool skeleton = false;
          for(auto &e : SkeletonTypes)
          {
            if(e.find(fd->getNameAsString()) != std::string::npos)
            {
              skeleton = true;
              std::string sfd = fd->getQualifiedNameAsString();
              if(sfd == "")
              {
                break;
              }

              size_t pos = sfd.find("impl::");
              if (pos != std::string::npos) {
                  sfd.erase(pos, 6); // 6 is the length of "impl::"
              }

              std::string label = fd->getQualifiedNameAsString() + "(";    
              size_t userfunctionArgAmount{0};

              for (auto& pair : SkeletonDescriptions) {
                if(pair.second.typeToString(pair.second.type).find(fd->getNameAsString()) == std::string::npos)
                {
                  userfunctionArgAmount = pair.second.userfunctionArgAmount;
                  break;
                }
              }

              for(size_t i{0}; i < userfunctionArgAmount; ++i)
              {
                label += "(*func"+ std::to_string(i)+")()";

                if(i + 1 < userfunctionArgAmount)
                {
                  label += ", ";
                }
              }
          ...
          //if(std::find(SkeletonTypes.begin(), SkeletonTypes.end(), fd->getNameAsString()) != SkeletonTypes.end())
          //{


          //}
          if(auto ftd = Candidate.getFunctionTemplate())
          {            
            SkepuArguments SA;

            std::string QualName = ftd->getQualifiedNameAsString();
            clangd::log("QualName: {0}", QualName);

           // clangd::log("SS: {0}",QualName);
            
            size_t doubleColonPos = QualName.find("::");
            size_t openAngleBracketPos = QualName.find('<');
            std::string implName = "";
            if (doubleColonPos != std::string::npos && openAngleBracketPos != std::string::npos) {
              implName = QualName.substr(doubleColonPos + 2, openAngleBracketPos - doubleColonPos - 2);
            }

           // clangd::log("the implname is {0}", implName);
            if(implName == "")
            {
              break;
            }

            size_t pos = implName.find("impl::");
            if (pos != std::string::npos) {
                implName.erase(pos, 6); // 6 is the length of "impl::"
            }

            auto it = SkeletonDescriptions.find(implName);
            if (it != SkeletonDescriptions.end()) {
              SA.type = SkeletonDescriptions[implName].name;
            } else {
              break;
            }

            std::size_t start = QualName.find('<');
            std::size_t end = QualName.rfind('>');
            int index{0};


            if(implName.find("Reduce") != std::string::npos && implName.find("Map") == std::string::npos)
            {  
              SA.arity = implName[implName.length() - 2];
              index = 2;
            }
            size_t userfunctionArgAmount = SkeletonDescriptions[implName].userfunctionArgAmount;
            int saIndex = userfunctionArgAmount + 2;
            if (start != std::string::npos && end != std::string::npos && end > start) 
            {
              std::string argumentsString = QualName.substr(start + 1, end - start - 1);
              ...
              {
                std::size_t first = argument.find_first_not_of(' ');
                std::size_t last = argument.find_last_not_of(' ');

                if (first != std::string::npos && last != std::string::npos)
                {
                  std::string param{argument.substr(first, last - first + 1)};
                  ...
                  {}
                  else if(index <= userfunctionArgAmount + 1 || multiRet)
                  {
                    std::string tmpParam = param;
                    if (tmpParam.find("tuple") != std::string::npos) {
                      // Found IndexND value;
                      size_t pos = tmpParam.find('<');
                      if (pos != std::string::npos) {
                        tmpParam = tmpParam.substr(pos + 1);
                      }
                      multiRet = true;
                    }

                    if (tmpParam.find(">") != std::string::npos) {
                      // Found IndexND value;
                      size_t pos = tmpParam.find('>');
                      if (pos != std::string::npos) {
                    ...
                    if(implName.find("Reduce") != std::string::npos)
                    {
                      index+= std::stoi(SA.arity);
                    }

                    SA.retValue.push_back(tmpParam);
                  }
                  //clangd::log("index: {0}, userfunctionArgAmount +2 : {1}", index, userfunctionArgAmount +2);
                  ...
                  if(index >= userfunctionArgAmount + 2 && !multiRet)
                  {
                    if (param.find("Index") != std::string::npos) {
                      // Found IndexND value;
                      SA.IndexND = param;
                      saIndex = index;
                    } 
                    else if(index <= saIndex + std::stoi(SA.arity))
                    {
                      std::string tmpParam = param;
                      if (tmpParam.find("Random") != std::string::npos) {
                        // Found Random value;
                        SA.randomArg = tmpParam;
                        continue;                        
                      }

                      if(implName.find("Reduce") != std::string::npos)
                      {
                        for(std::size_t i{0}; i<std::stoi(SA.arity); ++i)
                        {
                          SA.element_wise.push_back(tmpParam);
                        }
                      }
                      else
                      {
                    ...
                    else if(param.find("skepu::") != std::string::npos)
                    {
                      // Random-access containers
                      SA.random_access.push_back(param);
                    }
                    else 
                    {
                      SA.uniform.push_back(param);
                    }
                  }
                  index++;
                }
            ...
            clangd::log("SA: {0}", SA.to_string());

            int indexND = 0;
            if(SA.IndexND != "")
            {
              char second_to_last = SA.IndexND[SA.IndexND.length() - 2];
              indexND = second_to_last - '0';  
            }

            if(implName.find("Reduce") != std::string::npos)
            {
              char second_to_last = implName[implName.length() - 2];
              indexND = second_to_last - '0';  
            }

            //clangd::log("index: {0}", indexND);
            std::string container = "";
            if(indexND == 1)
            {
              container = "skepu::Vector";
            }
            else if(indexND == 2)
            {
              container = "skepu::Matrix";
            } else if(indexND == 3)
            {
              container = "skepu::Tensor3";
            } else if(indexND == 4)
            {
              container = "skepu::Tensor4";
            }
            ...
            if(SA.type == "Map" || SA.type == "MapPairs" 
            || SA.type == "MapPairsReduce" || SA.type == "MapOverlap1D"
            || SA.type == "MapOverlap2D" || SA.type == "MapOverlap3D"
            || SA.type == "MapOverlap4D")
            {
              for (auto elem : SA.retValue) {
                std::string indexStr{};

                if(container != "")
                {
                  indexStr = container + "<" + elem + ">& Return"+std::to_string(retCounter);
                }
                else
                {
                  indexStr = elem + "& Return"+ std::to_string(retCounter) +" | (Can be skepu container of " + elem + " type)";
                }
                retCounter++;

                all.push_back(indexStr);
              }
            
            }
            else 
            {
              retEnd = SA.retValue.front();
            }

            if(std::stoi(SA.arity) > 0)
            {
              int elemWiseCounter{0};
              for (auto elem : SA.element_wise) {
                std::string indexStr{};

                if(container != "")
                {
                  indexStr = container + "<" + elem + ">&elem"+std::to_string(elemWiseCounter);
                }
                else
                {
                  indexStr = elem + " elem"+ std::to_string(elemWiseCounter) +"  | (Can be skepu container of " + elem + " type)";
                }

                elemWiseCounter++;
                //indexStr = "skepu::Index"+ std::to_string(indexND) +"D<" + elem +">";
                all.push_back(indexStr);
              }
            }
            
            int randomCounter{0};
            for (auto iter : SA.random_access) {
              all.push_back(iter + " rand"+std::to_string(randomCounter));
              randomCounter++;
            }

            int uniformCounter{0};
            for (auto iter : SA.uniform) {
              all.push_back(iter + " uniform"+std::to_string(uniformCounter));
              uniformCounter++;
            }
            SigHelp.signatures.clear();

            SS.Signature.parameters.clear();
            std::string lbl{"skepu::"+ SA.type + "("};
            //std::string lbl{"skepu:: ("};
            ...
            for (auto iter = all.begin(); iter != all.end(); ++iter) {
              ...
              SS.Signature.parameters.push_back(pi);              
              if (iter != all.end() - 1) {
                lbl += ", ";
              } 
            ...
            //clangd::log("size: {0}", SS.Signature.parameters.size());
            ...
            SS.Signature.label = lbl;
          }  

          //clangd::log("currentArg: {0}", CurrentArg);
          SigHelp.signatures.clear();
          SigHelp.signatures.push_back(std::move(SS.Signature));
          SigHelp.activeParameter = CurrentArg;
          SigHelp.activeSignature = 0;
          break;
        }
      }
...
SignatureHelp signatureHelp(PathRef FileName, Position Pos,
  ...
  semaCodeComplete(
      std::make_unique<SignatureHelpCollector>(Options, DocumentationFormat,
                                               ParseInput.Index, Result, SigSkepu,
                                               SkeletonTypes, SkeletonDescriptions),
      Options,
      {FileName, *Offset, Preamble,
       PreamblePatch::createFullPatch(FileName, ParseInput, Preamble),
       ParseInput});
