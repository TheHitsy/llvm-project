...
HoverInfo getHoverContents(const NamedDecl *D, const PrintingPolicy &PP,
  ...
  {
    fillFunctionTypeAndParams(HI, D, FD, PP); // here goes mapreduce
  }
  ...
  if (!HI) {
    ...
    if (const SelectionTree::Node *N = ST.commonAncestor()) {
      ...
      if (const auto *DeclToUse = pickDeclToUse(Decls)) {
        ...
        maybeAddCalleeArgInfo(N, *HI, PP);
        bool runSkepu = true;
        if(isSkepuSkeleton(AST, DeclToUse, CurLoc.get()) && runSkepu)
        {
          std::string tmp{"auto"};
          HI->ReturnType = clangd::HoverInfo::PrintedType(tmp.c_str());
          const FunctionDecl *FD = getUnderlyingFunction(DeclToUse);

          std::string nameInfoS =  FD->getNameInfo().getAsString();
          size_t pos = nameInfoS.find("impl::");
          if (pos != std::string::npos) {
            nameInfoS.erase(pos, 6); // 6 is the length of "impl::"
          }

          std::string doc = "Skeleton of " + nameInfoS + ": ";

          for(auto &pa : AST.getSkeletonDescriptions())
          {
            if(pa.first.find(nameInfoS) != std::string::npos)
            {
              HI->Documentation = doc + pa.second;
              break;
            }
          }
          
          auto preHI = HI->Parameters.value();
          HI->Parameters.reset();
          HI->Parameters.emplace();

          for (const auto &param : preHI) {
            clangd::HoverInfo::PrintedType pt = param.Type.value();
            HoverInfo::Param out = param;
            out.Type = clangd::HoverInfo::PrintedType(pt.Type.c_str());
            HI->Parameters->emplace_back(out);
          }

          HI->Definition = "";
        }
        else if(Skeleton *skeleton = getSkepuDecl(AST, DeclToUse, CurLoc.get()))
        {
          //clangd::log("is skepu decl");
          if(runSkepu)
          {
            HI->ReturnType = clangd::HoverInfo::PrintedType(skeleton->resolvedReturnType.c_str());
            HI->Documentation = skeleton->getDescription();          
            HI->Parameters.emplace();
            HI->Definition = "";

            for(auto const& param : skeleton->resolvedParams)
            {
              HoverInfo::Param out;
              out.Type = clangd::HoverInfo::PrintedType(param.c_str());
              HI->Parameters->emplace_back(out);
            }
          }
...
bool isSkepuSkeleton(ParsedAST &AST, const clang::NamedDecl* Decl, clang::SourceLocation const& CurLoc)
{
  std::vector<std::string> vec = AST.getSkeletonTypes();
  for(auto &e : vec)
  {
    if(e.find(Decl->getNameAsString()) != std::string::npos)
    {
      if (const FunctionDecl *FD = getUnderlyingFunction(Decl))
      {
        if (const auto *NS = FD->getEnclosingNamespaceContext()) {
          if (auto *NSD = dyn_cast_or_null<NamespaceDecl>(NS)){
            return NSD->getQualifiedNameAsString() == "skepu";
          } 
        }
      }
    }
  ...
  return false;
  /*
  if(std::find(vec.begin(), vec.end(), Decl->getNameAsString()) != vec.end() )
  {
    if (const FunctionDecl *FD = getUnderlyingFunction(Decl))
    {
      if (const auto *NS = FD->getEnclosingNamespaceContext()) {
...
Skeleton* getSkepuDecl(ParsedAST &AST, const clang::NamedDecl* Decl, clang::SourceLocation const& CurLoc){
  ...
  for(auto skeleton : AST.getSkeletonFunctions())
  {
    ...
    if(skeleton->variableName != DeclName )
    {
      continue;
    }

    FullSourceLoc declLoc(CurLoc, sm);
    FullSourceLoc skelLoc(CurLoc, sm);
    if(skelLoc.getSpellingLineNumber() == declLoc.getSpellingLineNumber())
    {
      return skeleton;
    }
  }
