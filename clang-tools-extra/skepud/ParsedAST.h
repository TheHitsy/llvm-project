...
public:
  ...
  const std::vector<Skeleton*> getSkeletonFunctions() const
  {
    return Skeletons ;
  }

  const std::vector<std::string> getSkeletonTypes() const
  {
    return SkeletonTypes;
  }

  const std::unordered_map<std::string, std::string> getSkeletonDescriptions() const
  {
    return SkeletonDescriptions;
  }

  const std::unordered_map<std::string, Skeleton> getSkeletonStandard() const
  {
    return SkeletonsStandard;
  }

  const Skeleton getSkeletonTemplate(std::string name) const
  {
    for (const auto& pair : this->SkeletonsStandard) {
  ...
  const std::vector<UserFunction*> getUserfunctions() const
  {
    return Userfunctions;
  }

  const std::vector<UserType*> getUserTypes() const
  {
    return UserTypes;
  }

   const std::vector<UserConstant*> getUserConstant() const
  {
    return UserConstants;
  }

    const std::vector<SkepuContainer*> getSkepuContainers() const
  {
    return SkepuContainers;
  }
...
private:
  ...
  std::vector<std::string> SkeletonTypes;
  std::vector<UserFunction*> Userfunctions;
  std::vector<SkepuContainer*> SkepuContainers;
  std::unordered_map<std::string, std::string> SkeletonDescriptions;
  std::unordered_map<std::string, Skeleton> SkeletonsStandard;
