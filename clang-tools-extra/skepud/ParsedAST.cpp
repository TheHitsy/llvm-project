...
ParsedAST::build(llvm::StringRef Filename, const ParseInputs &Inputs,
  ...
  std::unordered_map<std::string, std::string> skeletonDescriptions{};
  ...
  std::unordered_map<std::string, Skeleton> SkeletonsStandard{};

  bool useSkepu = true;

  if(useSkepu)
  {
    SkepuMatcher skepuMatcher{Filename};
    if(skepuMatcher.runMatcher())
    {
      skeletons = std::move(skepuMatcher._Skeletons);
      skeletonTypes = skepuMatcher._SkeletonTypes;
      skeletonDescriptions = skepuMatcher._SkeletonDescriptions;
      userfunctions = std::move(skepuMatcher._Userfunctions);
      userTypes = std::move(skepuMatcher._UserTypes);
      userConstants = std::move(skepuMatcher._UserConstants);
      containers = std::move(skepuMatcher._Containers);
      SkeletonsStandard = skepuMatcher._SkeletonsStandard;
  ...
  ParsedAST Result(Filename, Inputs.Version, std::move(Preamble),
                   ...
                   std::move(skeletons),
                   std::move(userTypes),
                   std::move(userConstants),
                   std::move(skeletonTypes),
                   skeletonDescriptions,
                   std::move(userfunctions),
                   std::move(containers),
                   SkeletonsStandard);
  if (Result.Diags) {
...
ParsedAST::ParsedAST(PathRef TUPath, llvm::StringRef Version,
    ...
    : TUPath(TUPath), Version(Version), Preamble(std::move(Preamble)),
      ...
      UserTypes(std::move(userTypes)),
      UserConstants(std::move(userConstants)),
      SkeletonTypes(std::move(SkeletonTypes)),
      Userfunctions(std::move(userfunctions)),
      SkepuContainers(std::move(containers)) {
        
  //Userfunctions = userfunctions;
  SkeletonDescriptions = skeletonDescriptions;
  SkeletonsStandard = skeletonsStandard;
