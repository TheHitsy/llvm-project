...
void ClangdServer::signatureHelp(PathRef File, Position Pos,
  ...
  auto Action = [Pos, File = File.str(), CB = std::move(CB),
    ...
    clangd::SignatureHelp sh = clangd::signatureHelp(File, Pos, *PreambleData, ParseInput,
                              DocumentationFormat, SkeletonTypes, SkeletonDescriptions);
...
void ClangdServer::setSkepuCont(ParsedAST &AST)
{
  Skeletons.clear();
  ...
  for(auto par : AST.getSkeletonStandard())
  {
    SkeletonDescriptions[par.first] = par.second;
  }

  SkeletonTypes.clear();
  for(auto skeleType : AST.getSkeletonTypes())
  {
    SkeletonTypes.push_back(skeleType);
  }
...
void ClangdServer::inlayHints(PathRef File, std::optional<Range> RestrictRange,
  ...
  auto Action = [RestrictRange(std::move(RestrictRange)),
                 CB = std::move(CB), this](Expected<InputsAndAST> InpAST) mutable {
    if (!InpAST)
      return CB(InpAST.takeError());
...
void ClangdServer::documentSymbols(llvm::StringRef File,
  ...
  auto Action =
      [CB = std::move(CB), this](llvm::Expected<InputsAndAST> InpAST) mutable {
        if (!InpAST)
          return CB(InpAST.takeError());
